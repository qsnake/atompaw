...
!---- Write core Wave Function file for ABINIT
 if (prtcorewf) then
  file_abinit=TRIM(AEpot%sym)//'.'//TRIM(exctype)//'-corewf.abinit'
...
&   " - Generated by ",trim(atp_package), " v",trim(atp_version),&
&   " & AtomPAW2Abinit v",atompaw2abinitver
...
!--Read CORE SIZE
 pshead%core_size=0
 do ib=1,FC%norbit
  if (FC%iscore(ib)) pshead%core_size=pshead%core_size+1
 end do
...
!--Read MESH SIZE FOR CORE DENSITIES
 pshead%core_meshsz=coretailpoints

!--Read MESH SIZE FOR PSEUDO_VALENCE_DENSITY
 pshead%vale_meshsz=ivale

!--Read MESH SIZE FOR VLOCION
 pshead%vloc_meshsz=ivion
...
!------------------------------------------------------------------
 ...
 integer :: i_usexcnhat,i_prtcorewf,i_logspline,i_rsoptim,iend,ok
...
!------------------------------------------------------------------
 ...
 i_logspline=index(readline,'LOGSPLINE')
...
!--Read Kinetic term KINETIC_ENERGY_MATRIX
 ...
 do jlmn=1,pshead%lmn_size
  ...
  do ilmn=1,jlmn
   ...
   if (ilm==jlm) pawarray%kij(klmn)=half*PAW%kin(iln,jln) ! Conversion Ry->Ha
  enddo
 enddo
...
!!=================================================================
 ...
 subroutine wrpawps(pshead,pawps,pawarray,pawrad,loggrd,fname,funit)
