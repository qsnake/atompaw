MODULE excor
  ...
  INTEGER, PRIVATE, PARAMETER :: LIBXC  =18
  ...
  REAL(8), PRIVATE, PARAMETER :: mu = 0.2195149727645171d0
  REAL(8), PRIVATE, PARAMETER :: beta = 0.06672455060314922d0
  REAL(8), PRIVATE, PARAMETER :: gamm = 0.03109069086965489503494086371273d0
  REAL(8), PRIVATE, PARAMETER :: betabygamm=beta/gamm
...
&        +((1.d0+a1*rs)*((b1+3*b3*rs)/(2.d0*SQRT(rs))+b2+2*b4*rs))/(pprs*(pprs+1.d0/(2.d0*AA)))
  ...
  SUBROUTINE pbefunc(den,grad,fxc,dfxcdn,dfxcdgbg)
    ...
    dHdt = (2.d0*t*beta*gamm*(1.d0+2.d0*At2))/((gamm*ppt+beta*t*t*(1.d0+At2))*ppt)
    dHdtbg = ((2.d0*beta*gamm*(1.d0+ &
&        2.d0*At2))/((gamm*ppt+beta*t*t*(1.d0+At2))*ppt))/(2.d0*ks*n)
    !decdrs=-(2.d0*AA*a1)*ddlog(1.d0+1.d0/(2*AA*pprs)) &
    decdrs=-(2.d0*AA*a1)*term &
&        +((1.d0+a1*rs)*((b1+3*b3*rs)/(2.d0*SQRT(rs))+b2+2*b4*rs))/(pprs*(pprs+1.d0/(2.d0*AA)))
    dHdA=((2.d0+At2)*(At2*t*t*t*t*beta*gamm))/((gamm*ppt+beta*t*t*(1.d0+At2))*ppt)
...
&        - (7.d0/6.d0)*dHdt*t
    ...
    IF ((ABS(fxc).GT.1.d65).OR.(ABS(dfxcdn).GT.1.d65).OR.(ABS(dfxcdgbg).GT.1.d65)) THEN
...
&            fxc(Npts),stat=i)
  ...
  SUBROUTINE exch(Grid,den,rvxc,etxc,eexc,fin,v0,v0p)
    ...
    SELECT CASE(itype)
    CASE default
       WRITE(6,*) 'Error in exch -- itype not correct', itype
       STOP
    CASE(GGA_PBE) !!!!!!!PBE form!!!!!!
    ...
    CASE(LIBXC)  !!!!!!! External LibXC library!!!!!!
       IF (have_libxc) THEN
